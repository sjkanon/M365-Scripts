...
param (
    ...
    $DefaultAzureADgroup,
    [string]
    $LastpassAzureADgroup,
    [string]
    $ColigoAzureADGroup,
    [string]
    $allusersAzureADGroup
)
#####################################################################################
# Default Functions for retrieving configuration policy etc.. information via Graph API.
#####################################################################################
Function Get-DefaultAdministrativeTemplatePolicys(){
...
Function Get-DefaultConfigurationPolicys(){
 
  $graphApiVersion = "Beta"
  $Resource = "deviceManagement/deviceConfigurations"
  $uri = "https://graph.microsoft.com/$graphApiVersion/$($resource)"
  (Invoke-RestMethod -Uri $uri -Headers $AuthHeaders -Method Get).Value
  $confdef = (Invoke-RestMethod -Uri $uri -Headers $AuthHeaders -Method Get).Value
  #Selecting Default Configuration
  $confdef | Where-Object displayName -Like "Default"
}

Function Get-LastpassConfigurationPolicys(){
  ...
  $confdef = (Invoke-RestMethod -Uri $uri -Headers $AuthHeaders -Method Get).Value
  #Selecting Default Configuration
  $confdef | Where-Object displayName -contains "lastpass"

}

Function Get-notlastpassConfigurationPolicys(){
  ...
  $confdef = (Invoke-RestMethod -Uri $uri -Headers $AuthHeaders -Method Get).Value
  #Selecting Default Configuration
  $confdef | Where-Object displayName -contains "not lastpass"

}

Function Get-DefaultIntunePowershellscripts(){
...
Function Get-DefaultCompliancePolicys(){
...
Function Get-DefaultSecurityBaseLinePolicys(){
...
#Default Assign Delivery Optimization, Wifi, Update Rings, Endpoint protection, Custom
$ConfigurationPolicys = Get-DefaultConfigurationPolicys
...
foreach ($Configurationpolicy in $ConfigurationPolicys){
  ...
  $JSON = "{'assignments':[{'id':'','target':{'@odata.type':'#microsoft.graph.groupAssignmentTarget','groupId':'$($DefaultDefaultAzureADgroup)'}}]}"
...
foreach ($AdministrativePolicy in $AdministrativePolicys){
  ...
  $JSON = "{'assignments':[{'id':'','target':{'@odata.type':'#microsoft.graph.groupAssignmentTarget','groupId':'$($DefaultAzureADgroup)'}}]}"
  Invoke-RestMethod -Uri $policyuri -Headers $AuthHeaders -Method Post -Body $JSON -ErrorAction Stop -ContentType 'application/json'
}
...
foreach ($IntunePowershellScript in $IntunePowershellScripts){
  ...
  $JSON = "{'deviceManagementScriptGroupAssignments':[{'@odata.type':'#microsoft.graph.deviceManagementScriptGroupAssignment','targetGroupId': '$DefaultAzureADgroup','id': '$policyid'}]}"
  Invoke-RestMethod -Uri $policyuri -Headers $AuthHeaders -Method Post -Body $JSON -ErrorAction Stop -ContentType 'application/json'
}
