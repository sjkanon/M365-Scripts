...
Everything the script does hangs off two facts gathered read-only in steps 1 and 2: **is the client behind**, and **is the add-in present**.

```
installed build vs published build          add-in present?
            |                                     |
            +------------------+------------------+
                               |
      +------------------------+------------------------+
  ...
  (or -Force)              add-in missing           add-in present
      |                        |                        |
  full reinstall          add-in only              nothing at all
  steps 3-7               step 6-7 only            exit 0, no output
                          (3-5 skipped)            in -Quiet mode
```
...
|---|------|--------------|-----------|
| 1 | Preflight | Enumerate `*MSTEAMS*` AppX packages for all users, find the meeting add-in in both uninstall hives, note running Teams/Outlook | read-only |
| 2 | Version check | Ask the Teams config service for the published build, compare, decide | read-only |
| 3 | Download | Create the working folder, download `teamsbootstrapper.exe`, check size and Authenticode signature | guarded |
| 4 | Uninstall | `msiexec /x` the add-in, `Remove-AppxPackage -AllUsers`, `Remove-AppxProvisionedPackage` | guarded |
| 5 | Install | `teamsbootstrapper.exe -p` (provision for all users) | guarded |
| 6 | Add-in | Locate `MicrosoftTeamsMeetingAddinInstaller.msi` inside the installed package, install it with `ALLUSERS=1` | guarded |
| 7 | Verify | Re-read the add-in registration and the provisioned package, compare against the published build | reported as skipped |
...
Steps 3–5 are skipped entirely when the client is current and only the add-in is missing.
...
Accepted truthy values: `true`, `1`, `yes`. A parameter passed on the command line always wins over the environment.
