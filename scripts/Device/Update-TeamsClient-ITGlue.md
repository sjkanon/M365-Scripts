...
| `Teams Meeting Add-in install failed (exit code 1638)` | Er staat nog een **andere** add-in geregistreerd. Niet de versie is bepalend maar het feit dát er iets geregistreerd staat: gemeten met een nieuwere MSI (`1.26.21803`) die weigerde over een oudere `1.25.28902` | De oude eerst verwijderen. Kan Windows Installer dat niet meer (`1612`), dan `-ClearOrphanedAddInRegistration` | L3 |
| `Add-in X is still registered, so installing Y would answer 1638` | Zo bedoeld: het script laat de bestaande add-in staan in plaats van hem te slopen | Add-in verwijderen via Programma's en onderdelen en opnieuw draaien. Weigert Windows Installer dat (`1612`), dan `-ClearOrphanedAddInRegistration` gebruiken | L3 |
